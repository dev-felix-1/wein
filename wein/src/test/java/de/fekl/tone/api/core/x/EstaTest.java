...
public class EstaTest {
	...
	public void testStateContainerWithNet() {
		ITokenStore<IToken> tokenState = new SimpleTokenStore<>();
		System.err.println(ITokenStore.print(tokenState));

		SimpleStateContainer<ITokenStore<IToken>> simpleStateContainer = new SimpleStateContainer<>(tokenState);
		simpleStateContainer.changeState(current -> {
			ITokenStore<IToken> newState = new SimpleTokenStore<>();
