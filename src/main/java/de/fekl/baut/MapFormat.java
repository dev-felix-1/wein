...
public class MapFormat extends Format {
	 ...
	 * returns converted string. It scans <code>pattern</code> for {} brackets, then
	 * parses enclosed string and replaces it with argument's <code>get()</code>
	 * value.
	 * 
	...
	/**
	 * Returns the value for given key. Subclass may define its own beahvior of this
	 * method. For example, if key is not defined, subclass can return <not defined>
	 * string.
	 *
	 * @param key Key.
	 ...
	 *                                     parser found unmatched brackets (this
	 *                                     exception should be switched off using
	 *                                     setExactMatch(false)).
	 */
	public String processPattern(String newPattern) throws IllegalArgumentException {
		...
		while (true) {
			...
			if (ridx < 0) {
				if (exactmatch) {
					throw new IllegalArgumentException("unmachted braces found" + " " + lidx);
				} else {
					break;
				}
			}
			...
			if (obj == null) {
				if (throwex) {
					throw new IllegalArgumentException(
							MessageFormat.format("MSG_FMT_ObjectForKey", new Object[] { new Integer(key) }));
				} else {
					obj = ldel + key + rdel;
				}
			}
